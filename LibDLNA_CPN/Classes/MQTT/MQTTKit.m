...
static void on_disconnect(struct mosquitto *mosq, void *obj, int rc)
{
    ...
    if (rc == 0) {
        ...
        if (client.connectionLostHandler) {
            client.connectionLostHandler(rc);
        }
    }
...
- (void) connectWithCompletionHandler:(void (^)(MQTTConnectionReturnCode code))completionHandler {
    ...
    int rc;
    if (cstrUsername != NULL && cstrPassword != NULL) {
        rc = mosquitto_username_pw_set(mosq, cstrUsername, cstrPassword);
        if (rc != MOSQ_ERR_SUCCESS) {
            LogDebug(@"set username and pwd error:%d", rc);
        }
    }

    rc = mosquitto_reconnect_delay_set(mosq, self.reconnectDelay, self.reconnectDelayMax, self.reconnectExponentialBackoff);
    if (rc != MOSQ_ERR_SUCCESS) {
        LogDebug(@"set reconnect delay error:%d", rc);
    }

    rc = mosquitto_connect(mosq, cstrHost, self.port, self.keepAlive);
    LogDebug(@" mqtt connect host: %@, port: %d, status:%d", self.host, self.port, rc);
    if (rc != MOSQ_ERR_SUCCESS) {
        LogDebug(@"on_connect error:%d", rc);
        self.connected = NO;
        //mosquitto_connack_string(rc); //reference from  mosquitto_connack_string function
        if (self.connectionCompletionHandler) {
            self.connectionCompletionHandler([self mosquittoConnack:rc]);
        }
        return;
    }
...
- (MQTTConnectionReturnCode)mosquittoConnack:(int)connack_code{
    switch(connack_code){
        case 0:
            return ConnectionAccepted;
        case 1:
            return ConnectionRefusedUnacceptableProtocolVersion;
        case 2:
            return ConnectionRefusedIdentiferRejected;
        case 3:
            return ConnectionRefusedServerUnavailable;
        case 4:
            return ConnectionRefusedBadUserNameOrPassword;
        case 5:
            return ConnectionRefusedNotAuthorized;
        default:
            return ConnectionUnkownError;
    }
}
